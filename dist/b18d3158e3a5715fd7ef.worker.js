...
/******/ 	return __webpack_require__(__webpack_require__.s = "./node_modules/@ngtools/webpack/src/index.js?{\"tsConfigPath\":\"src/tsconfig.app.json\"}!./src/app/worker/worker.ts");
/******/ })
/************************************************************************/
/******/ ({

/***/ "./node_modules/@ngtools/webpack/src/index.js?{\"tsConfigPath\":\"src/tsconfig.app.json\"}!./src/app/worker/worker.ts":
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__exceptions__ = __webpack_require__("./src/app/exceptions.ts");
...
class SchedulingWorker {
    constructor() {
         ...
         */
        this.scheduleCourses = (function* (bigSectionsAlreadyTyped, index, chosenSections) {
            // base case
            if (index == bigSectionsAlreadyTyped.length) {
                ...
                for (let courseCombination of iter) {
                    yield* this.scheduleCourses(bigSectionsAlreadyTyped, index + 1, chosenSections.concat(courseCombination));
                }
            }
        }).bind(this);
    }
     ...
     * @param m2
     */
    meetingOverlap(m1, m2) {
        const dateIntersect = __WEBPACK_IMPORTED_MODULE_0_lodash__["intersection"](m1.date.split(''), m2.date.split(''));
     ...
     */
    groupClassSectionByBigSectionException(course, exceptionType) {
     ...
     * @param course
     */
    groupClassSectionByBigSection(course) {
        // special cases handling
        const deptName = course.name.split(" ")[0];
        for (const exceptionType in __WEBPACK_IMPORTED_MODULE_2__exceptions__["a" /* EXCEPTIONS */]) {
            if (deptName in __WEBPACK_IMPORTED_MODULE_2__exceptions__["a" /* EXCEPTIONS */][exceptionType]) {
        ...
        // "normal case"
        const sections = __WEBPACK_IMPORTED_MODULE_0_lodash__["groupBy"](course.sections, sec => sec.section[0]);
        return __WEBPACK_IMPORTED_MODULE_0_lodash__["values"](sections);
    }
    /**
     * group the sections based on their "type". i.e. "LEC", "DIS", "LCD", "LBD"......
     * @param sections
     */
    groupClassSectionBySectionType(sections) {
        const sectionGroups = __WEBPACK_IMPORTED_MODULE_0_lodash__["groupBy"](sections, sec => sec.type);
        return __WEBPACK_IMPORTED_MODULE_0_lodash__["values"](sectionGroups);
    }
    /**
     ...
     * @param section
     */
    createBigSectionGenerator(bigSectionAlreadyTypedPre) {
        ...
        const gen = function* () {
            const doCartesian = (function* (i, prod) {
     ...
     */
    pruneBigSection(bigSectionAlreadyTyped, sectionsChosen) {
        return bigSectionAlreadyTyped.map(bsat => bsat.filter(sec => !sectionsChosen.some(csec => this.sectionOverlap(sec, csec))));
    }
    /**
     * returns a generator that could only generate the "optimal" schedule
     * @param courses
     */
    createStateMachineOnlyOptimal(courses) {
        const bigSectionIndices = new Array(courses.length).fill(0);
        const bigSectionsOfCourses = [];
        ...
        var gen = function* () {
            while (true) {
                ...
                const chosenBigSectionsAlreadyTyped = bigSectionsOfCourses.map((bsoc, index) => this.groupClassSectionBySectionType(bsoc[bigSectionIndices[index]]));
                // generate optimal schedule given current set of type-grouped big sections
                yield* this.scheduleCourses(chosenBigSectionsAlreadyTyped, 0, []);
                // get exhaustive on big section combinations
                ++bigSectionIndices[courses.length - 1];
                for (let j = courses.length; --j >= 0 && bigSectionIndices[j] == bigSectionLengths[j];) {
                    if (j == 0) {
     ...
     * @param courses
     */
    createStateMachine(courses, filterInfo) {
...
/* harmony export (immutable) */ __webpack_exports__["SchedulingWorker"] = SchedulingWorker;
...
self.onmessage = (e) => {
    ...
    if (payload.reset) {
        ...
        const filterInfo = payload.filterInfo;
...
throw new Error("Module build failed: Error: ENOENT: no such file or directory, open '/Users/dwd/Documents/Holden's_Codes/uiuc-schedule-builder/node_modules/moment-timezone/index.js'\n    at Error (native)");
...
/* harmony export (immutable) */ __webpack_exports__["a"] = EXCEPTIONS;
...
/******/ });
//# sourceMappingURL=b18d3158e3a5715fd7ef.worker.js.map